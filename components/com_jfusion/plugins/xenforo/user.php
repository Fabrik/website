...
class JFusionUser_xenforo extends JFusionUser
{
	...
	function &getUser($userinfo, $identifier_type = 'auto', $ignore_id = 0)
	{
		...
		*/
		...
		if ($result)
		{
			...
			// @TODO gives notice when editing user
	...
	function updateUser($userinfo, $overwrite = 0)
	{
		...
		if (empty($status['error']))
		{
			...
			$helper->changePrimaryUserGroup($existinguser->userid, 2, JText::_('ACTIVATION_UPDATE_ERROR'), $status, false, true);
			$helper->changeSecondaryUserGroup($existinguser->userid, $userGroups, JText::_('ACTIVATION_UPDATE_ERROR'), $status, false);
	...
	*/
	...
	function activateUser($userinfo, &$existinguser, &$status)
	{
		...
		// Update the user
		$helper->changePrimaryUserGroup($existinguser->userid, $params->get('usergroup'), JText::_('ACTIVATION_UPDATE_ERROR'), $status, false);
